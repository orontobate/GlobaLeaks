...
def _db_serialize_archived_questionnaire_schema(session, aqs, language):
...
def db_serialize_archived_questionnaire_schema(session, hash, language):
    aqs = session.query(models.ArchivedSchema).filter(models.ArchivedSchema.hash == hash, models.ArchivedSchema.type == u'questionnaire').one()

    return _db_serialize_archived_questionnaire_schema(session, aqs, language)


def db_serialize_archived_preview_schema(session, aqs, language):
    return _db_serialize_archived_questionnaire_schema(session, aqs, language)
...
def db_serialize_questionnaire_answers(session, tid, usertip, internaltip):
    questionnaire = db_serialize_archived_questionnaire_schema(session, internaltip.questionnaire_hash, State.tenant_cache[tid].default_language)
...
def db_archive_questionnaire_schema(session, tid, questionnaire, questionnaire_hash):
...
def serialize_itip(session, internaltip, language):
    return {
        ...
        'questionnaire': db_serialize_archived_questionnaire_schema(session, internaltip.questionnaire_hash, language),
...
def db_create_submission(session, tid, request, uploaded_files, client_using_tor):
    ...
    db_archive_questionnaire_schema(session, tid, steps, questionnaire_hash)
