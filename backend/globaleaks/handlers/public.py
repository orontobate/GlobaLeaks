...
def db_serialize_node(session, tid, language):
    ...
    misc_dict = {
        'languages_enabled': l10n.EnabledLanguage.list(session, tid) if ro_node['wizard_done'] else list(LANGUAGES_SUPPORTED_CODES),
...
@transact
def serialize_node(session, tid, language):
    return db_serialize_node(session, tid, language)
