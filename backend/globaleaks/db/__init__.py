...
def db_refresh_tenant_cache(session, tid_list):
    ...
    """
    result_set = session.query(Config).filter(Config.tid.in_(tid_list)).order_by(Config.tid, Config.var_name)
    ...
    for tid, lang in models.l10n.EnabledLanguage.tid_list(session, tid_list):
