...
def receiver_serialize_receiver(session, tid, receiver, user, language):
    ...
    contexts = [id for id in session.query(models.ReceiverContext.context_id) \
...
def get_receivertip_list(session, tid, receiver_id, language):
    ...
    for rtip in rtips:
        ...
        rtip_summary_list.append({
            ...
            'preview_schema': db_serialize_archived_preview_schema(session, archivedschema, language),
